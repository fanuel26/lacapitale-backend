...
const DetailVente_1 = __importDefault(global[Symbol.for('ioc.use')]("App/Models/DetailVente"));
const ResponseBody_1 = __importDefault(global[Symbol.for('ioc.use')]("App/Models/ResponseBody"));
const Vente_1 = __importDefault(global[Symbol.for('ioc.use')]("App/Models/Vente"));
const VenteRegistrationValidator_1 = __importDefault(global[Symbol.for('ioc.use')]("App/Validators/VenteRegistrationValidator"));
class VentesController {
    async list({ response }) {
        const produit = await Vente_1.default.all();
    ...
    async listById({ request, response }) {
        try {
            const produit = await Vente_1.default.findOrFail(request.params().id);
    ...
    async detailVenteByIdVente({ request, response }) {
        try {
            const produit = await Database_1.default.from("detail_ventes")
    ...
    async save({ request, response }) {
        const data = await request.validate(VenteRegistrationValidator_1.default);
        console.log(data);
        ...
        if (addVentes.length != 0) {
            const produit = new Vente_1.default();
            ...
            try {
                ...
                for (let i = 0; i < addVentes.length; i++) {
                    const av = new DetailVente_1.default();
    ...
    async update({ request, response }) {
        try {
            await Vente_1.default.query()
                .where("id", request.params().id)
                .update(request.body());
            const produit_value = await Vente_1.default.query().where("id", request.params().id);
            return response.accepted({
                status: true,
                data: produit_value,
...
exports.default = VentesController;
//# sourceMappingURL=VentesController.js.map